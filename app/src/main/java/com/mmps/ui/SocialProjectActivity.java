...
public class SocialProjectActivity extends BaseActivity {
    ...
    private RecyclerView rvSocialProject;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_social_project);
    ...
    private void initui() {
        ...
        rvSocialProject = findViewById(R.id.rvSocialProjects);
        rvSocialProject.setLayoutManager(new LinearLayoutManager(this));

        if (getIntent().hasExtra("library")) {
            libraryResponseModel = (SmartLibraryResponseModel) getIntent().getExtras().get("library");
            title.setText(getString(R.string.social_projects));
            ...
            try {
                ...
                btnSearch.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        if (SOAPUtils.isNetworkConnected(SocialProjectActivity.this)) {
                            if (!etSearchBook.getText().toString().isEmpty()) {
                                ...
                                new LibraryTasks(URLConstants.ACTION_SEARCH_BOOK, map, SocialProjectActivity.this).execute();
                            } else {
                                etSearchBook.setError(getString(R.string.cannot_be_empty));
                            }
                        } else {
                            Toast.makeText(SocialProjectActivity.this, getString(R.string.no_internet_connection), Toast.LENGTH_LONG).show();
                        }
                    }
                });

                if (getIntent().hasExtra("social")) {
                    ArrayList<SocialProjectModel> socialProjectModels = (ArrayList<SocialProjectModel>) getIntent().getExtras().get("social");
                    rvSocialProject.setAdapter(new RecyclerAdapter(SocialProjectActivity.this, socialProjectModels));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
